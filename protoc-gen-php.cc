 ...
 * Start <field>+ (You have to know what type of Message it is, and it is not
 * length prefixed)
 *
 * The Message class should not print its own length (this should be printed by
 * the parent Message)
 * The Group class should only print its field, the parent should print the
 * start/end tag
...
void PHPFileGenerator::PrintFields(const Descriptor& message) {
  if (!options_.skip_unknown()) printer_.Print("\nprivate $_unknown;\n");
